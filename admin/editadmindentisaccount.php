...
if(isset($_POST['submit'])){
 ...
 if($pass1 == $pass2 && $pass1!=null){
  ...
  $sql = "UPDATE `tblloginuser` SET `firstname` = '$fname', `lastname` = '$lname', `contact` = '$contact', `username` = '$username', `password` = '$pass', `Access` = '$access' WHERE `tblloginuser`.`ID` = '$id';";
...
$sql = "UPDATE `tblloginuser` SET `firstname` = '$fname', `lastname` = '$lname', `contact` = '$contact', `username` = '$username',`Access` = '$access' WHERE `tblloginuser`.`ID` = '$id';";
$result = mysqli_query($con, $sql);
echo '<script>alert("Data Successfully Updated!");';
     echo 'window.location.replace("viewadmindentistaccount.php");</script>';
                ...
                <div class="container-fluid">
                    ...
                    <div class="card shadow mb-4">
                        <div class="card-header py-3">
                            <h6 class="m-0 font-weight-bold text-primary">ADD User Account</h6>
                        </div>
 <?php
         $id=$_GET['ID'];
        $sql = "SELECT * FROM tblloginuser WHERE ID = '$id'";
        $result = mysqli_query($con,$sql);
